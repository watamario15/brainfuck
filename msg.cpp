  ...
  switch (id) {
    case IDC_CMDBTN_FIRST:  // Run
      ...
      if (runner::hThread) {
        ...
        util::messageBox(hWnd, global::hInst, L"Failed to create a runner thread.",
                         L"Internal Error", MB_ICONERROR);
        break;
      }
    ...
    case IDC_CMDBTN_FIRST + 1: {  // Next
      ...
      if (result == Brainfuck::RESULT_ERR) {
        util::messageBox(hWnd, global::hInst, runner::bf.getLastError(), L"Brainfuck Error",
                         MB_ICONWARNING);
      }
    ...
    case IDM_BF_BREAKPOINT:
      ...
      runner::bf.setBehavior(global::noInput, global::wrapInt, global::signedness,
                             global::breakpoint);
      break;
    ...
    default:
      if (id >= IDC_SCRKBD_FIRST && id < IDC_SCRKBD_FIRST + SCRKBD_LEN &&
...
BOOL onCreate(HWND hWnd, LPCREATESTRUCT lpCreateStruct) {
  ...
  for (i = 0; i < CMDBTN_LEN; ++i) {
    global::hCmdBtn[i] = CreateWindowExW(
        0, L"BUTTON", wcCmdBtn[i],
        WS_CHILD | WS_VISIBLE | BS_PUSHBUTTON | (i == 2 || i == 3 ? WS_DISABLED : 0), 0, 0, 0, 0,
        hWnd, (HMENU)(IDC_CMDBTN_FIRST + i), global::hInst, NULL);
  }

  // Screen keyboard
  for (i = 0; i < SCRKBD_LEN; ++i) {
    global::hScrKB[i] =
        CreateWindowExW(0, L"BUTTON", wcScrKB[i], WS_CHILD | WS_VISIBLE | BS_PUSHBUTTON, 0, 0, 0, 0,
  ...
  if (global::cmdLine[0]) {
    ui::openFile(false, global::cmdLine);
  } else {
    ui::openFile(true);
  }
...
void onInitMenuPopup(HWND hWnd, HMENU hMenu, UINT item, BOOL fSystemMenu) {
  ...
  CheckMenuRadioItem(hMenu, IDM_BF_MEMTYPE_SIGNED, IDM_BF_MEMTYPE_UNSIGNED,
                     global::signedness ? IDM_BF_MEMTYPE_SIGNED : IDM_BF_MEMTYPE_UNSIGNED,
                     MF_BYCOMMAND);

  // Brainfuck -> Output Charset
  CheckMenuRadioItem(hMenu, IDM_BF_OUTPUT_UTF8, IDM_BF_OUTPUT_HEX, global::outCharSet,
                     MF_BYCOMMAND);
  ...
  CheckMenuRadioItem(hMenu, IDM_BF_NOINPUT_ERROR, IDM_BF_NOINPUT_FF,
                     IDM_BF_NOINPUT_ERROR + global::noInput, MF_BYCOMMAND);

  // Brainfuck -> Integer Overflow
  CheckMenuRadioItem(hMenu, IDM_BF_INTOVF_ERROR, IDM_BF_INTOVF_WRAPAROUND,
                     global::wrapInt ? IDM_BF_INTOVF_WRAPAROUND : IDM_BF_INTOVF_ERROR,
                     MF_BYCOMMAND);

  // Brainfuck -> Breakpoint
  CheckMenuItem(hMenu, IDM_BF_BREAKPOINT,
                MF_BYCOMMAND | global::breakpoint ? MF_CHECKED : MF_UNCHECKED);
  ...
  // Options -> Word Wrap
  CheckMenuItem(hMenu, IDM_OPT_WORDWRAP,
                MF_BYCOMMAND | global::wordwrap ? MF_CHECKED : MF_UNCHECKED);
...
void onSize(HWND hWnd) {
  ...
  if (global::horizontal) {
    ...
    MoveWindow(global::hOutput, scrX / 3, halfY + topEditor, scrX / 3, scrY - halfY - topEditor,
               TRUE);
    MoveWindow(global::hMemView, scrX * 2 / 3, topEditor, scrX - scrX * 2 / 3, scrY - topEditor,
               TRUE);
  } else {
